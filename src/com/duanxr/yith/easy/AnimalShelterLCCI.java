...
public class AnimalShelterLCCI {
  ...
  class AnimalShelf1 {
    ...
    Queue<Pair<Long, int[]>> cats;
    Queue<Pair<Long, int[]>> dogs;
    int[] ghost;
    ...
    public void enqueue(int[] animal) {
      if (animal[1] == 0) {
        cats.offer(new Pair<>(++id, animal));
      } else {
        dogs.offer(new Pair<>(++id, animal));
      }
    }
    ...
    public int[] dequeueAny() {
      ...
      if (dogs.peek().getKey() > cats.peek().getKey()) {
        return dequeueCat();
      }
      return dequeueDog();
    }
