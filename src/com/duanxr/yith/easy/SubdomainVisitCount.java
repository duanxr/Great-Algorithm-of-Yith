  ...
  class Solution {
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
      ...
      for (String cpdomain : cpdomains) {
        ...
        int count = Integer.parseInt(cps[0]);
        String d = cps[1];
        String[] part = d.split("\\.");
        String now = null;
        for (int i = part.length - 1; i >= 0; i--) {
          if (now == null) {
            ...
            stringBuilder.append(part[i]).append('.').append(now);
            now = stringBuilder.toString();
          }
          if (!domain.containsKey(now)) {
            domain.put(now, count);
          } else {
            domain.put(now, domain.get(now) + count);
          }
        }
      }
