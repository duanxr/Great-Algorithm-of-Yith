...
public class UniqueEmailAddresses {
   ...
   * Output: 2 Explanation: "testemail@leetcode.com" and "testemail@lee.tcode.com" actually receive
   * mails
   *
   *
   * Note:
   *
   * 1 <= emails[i].length <= 100 1 <= emails.length <= 100 Each emails[i] contains exactly one '@'
   * character.
   *
   ...
   *
   * 如果在电子邮件地址的本地名称部分中的某些字符之间添加句点（'.'），则发往那里的邮件将会转发到本地名称中没有点的同一地址。例如，"alice.z@leetcode.com” 和
  ...
  class Solution {
    ...
    public int numUniqueEmails(String[] emails) {
      ...
      for (String email : emails) {
        ...
        if (local.contains("+")) {
          local = local.substring(0, local.indexOf("+") - 1);
        }
        String e = local + split[1];
        strings.add(e);
      }
      return strings.size();
    }
